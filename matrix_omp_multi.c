...
// Recursive determinant using cofactor expansion
 double det_recursive_hybrid(double **M, int n) {
    ...
    for(int f=0; f<n; f++){
        ...
        get_cofactor_hybrid(M, n, minor, 0, f);

        int child_index = get_free_child_hybrid();
...
double matrix_determinant_hybrid(Matrix M)
{
    ...
    return det_recursive_hybrid(M.data, M.rows);
}
...
Matrix matrix_multiply_hybrid(Matrix x, Matrix y)
{
    ...
    {
        int child_indix = get_free_child_hybrid();
        //// i am here
...
//---------------------------------------------------
//Eigenfunction Hybrid (MP for row distribution, OpenMP for vector reduction)
EigenResult matrix_eigen_hybrid(Matrix M)
{
    ...
    if (child_count < n) {
        ...
        for (int i = child_count; i < n; i++) {
            create_child_hybrid();                 // creates one child and adds it to the pool
        }
    }
    ...
    double norm = 0.0;
    for (int i = 0; i < n; i++) {
        x.data[i][0] = (double)rand() / RAND_MAX + 0.3;
        norm += x.data[i][0] * x.data[i][0];
    }
    norm = sqrt(norm);
    for (int i = 0; i < n; i++) x.data[i][0] /= norm;

    printf("\nPower Method (Hybrid MP/OpenMP) - Size %dx%d\n", n, n);
    ...
    for (int iter = 0; iter < max_iter; iter++)
    {
        ...
        // Send one row-computation task to each child (MP part)
        for (int row = 0; row < n; row++)
        {
            int idx = get_free_child_hybrid(); 
            child_idx[row] = idx;
